...
my_list <- list(self_efficacy_cig = -1,
                ...
                completed_app_usage_preblock = -1,
                any_response_2qs = -1)
```
...
row.names(dat_out) <- c("Treatment (Prompt = 1, No Prompt = 0)", "Treatment x Prior Self Efficacy", row.names(dat_result_control))
...
fit1 <- wcls(
  ...
  control_formula = ~ days_between_v1_and_coinflip_local + self_efficacy_cig_lag1 + any_response_2qs, 
  availability = "overall_elig"
)
...
row.names(dat_out) <- c("Treatment (Prompt = 1, No Prompt = 0)", "Treatment x Any Response to Two-Question Survey", paste("Any Response to Two-Question Survey: ", 0:1, sep = ""), row.names(dat_result_control))

# Format output to 5 decimal places
dat_out %>% round(., 5) %>% format(., nsmall = 5) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "primary_moderator_any_response_2qs.csv"), row.names = TRUE)
```
...
fit1 <- wcls(
  ...
  control_formula = ~ days_between_v1_and_coinflip_local + self_efficacy_cig_lag1 + any_response_2qs + I(self_efficacy_cig_lag1 * any_response_2qs), 
  availability = "overall_elig"
)
...
row.names(dat_out) <- c("Treatment (Prompt = 1, No Prompt = 0)", 
                        ...
                        row.names(dat_result_control))

# Format output to 5 decimal places
dat_out %>% round(., 5) %>% format(., nsmall = 5) %>% write.csv(., file.path("analysis-complete-case", "formatted-output", "primary_moderator_prior_self_efficacy_and_any_response_2qs.csv"), row.names = TRUE)
```
