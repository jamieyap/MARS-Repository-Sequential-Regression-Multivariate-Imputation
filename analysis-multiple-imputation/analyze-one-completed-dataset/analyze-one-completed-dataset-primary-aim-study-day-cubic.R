...
fit1 <- tryCatch(expr = {wcls(
  ...
  control_formula = ~ 1 + days_between_v1_and_coinflip_local + I(days_between_v1_and_coinflip_local * days_between_v1_and_coinflip_local) + I(days_between_v1_and_coinflip_local * days_between_v1_and_coinflip_local * days_between_v1_and_coinflip_local) + motivation_cig_lag1 + Y_lag1 + cigarette_counts_lag1, 
...
saveRDS(fit1, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("fit_obj_primary_study_day_cubic.rds", sep = "")))
saveRDS(results_obj, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_primary_study_day_cubic.rds", sep = "")))
...
for(idx_replicate in 1:max_replicate_id){
  ...
  saveRDS(fit1, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("fit_obj_primary_study_day_cubic", "_replicate_", idx_replicate, ".rds", sep = "")))
  saveRDS(results_obj, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_primary_study_day_cubic", "_replicate_", idx_replicate, ".rds", sep = "")))
}
