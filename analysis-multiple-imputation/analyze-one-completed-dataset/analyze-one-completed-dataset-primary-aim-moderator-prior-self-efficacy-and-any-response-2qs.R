...
fit1 <- tryCatch(expr = {wcls(
  ...
  treatment = "coinflip",
  rand_prob = 0.5,
  moderator_formula = ~ self_efficacy_cig_lag1 + any_response_2qs + I(self_efficacy_cig_lag1 * any_response_2qs),  
  control_formula = ~ 1 + days_between_v1_and_coinflip_local + self_efficacy_cig_lag1 + any_response_2qs + I(self_efficacy_cig_lag1 * any_response_2qs), 
...
saveRDS(fit1, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("fit_obj_primary_moderator_prior_self_efficacy_and_any_response_2qs.rds", sep = "")))
saveRDS(results_obj, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_primary_moderator_prior_self_efficacy_and_any_response_2qs.rds", sep = "")))
...
for(idx_replicate in 1:max_replicate_id){
  ...
  saveRDS(fit1, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("fit_obj_primary_moderator_prior_self_efficacy_and_any_response_2qs", "_replicate_", idx_replicate, ".rds", sep = "")))
  saveRDS(results_obj, file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_primary_moderator_prior_self_efficacy_and_any_response_2qs", "_replicate_", idx_replicate, ".rds", sep = "")))
}
