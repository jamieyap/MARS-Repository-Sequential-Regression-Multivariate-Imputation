...
results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", 1, "results_obj_primary_moderator_prior_self_efficacy_and_any_response_2qs.rds"))
...
for(j in 1:num_terms){
  ...
  for(mi_dataset_num in 1:.__total_imputed_datasets){
    results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, "results_obj_primary_moderator_prior_self_efficacy_and_any_response_2qs.rds"))
...
row.names(fit_pooled) <- c("Treatment (Prompt = 1, No Prompt = 0)", 
                           "Treatment x Prior Self-Efficacy",
                           "Treatment x Any Response to Two-Question Survey", 
                           "Treatment x Prior Self-Efficacy x Any Response to Two-Question Survey",
                           "Contrast: Prior Self-Efficacy = 0, Moderated Effect of Any Response to Two-Question Survey",
                           "Contrast: Prior Self-Efficacy = 1, Moderated Effect of Any Response to Two-Question Survey",
                           "Contrast: Prior Self-Efficacy = 2, Moderated Effect of Any Response to Two-Question Survey",
                           "Contrast: Prior Self-Efficacy = 3, Moderated Effect of Any Response to Two-Question Survey",
                           "Contrast: Prior Self-Efficacy = 4, Moderated Effect of Any Response to Two-Question Survey")
fit_pooled_causal <- fit_pooled
print(fit_pooled_causal)

dat_pool_stats <- bind_rows(list_pool_stats)
row.names(dat_pool_stats) <- row.names(fit_pooled)
...
for(mi_dataset_num in 1:.__total_imputed_datasets){
  ...
  for(idx_replicate in 1:.__par_total_replicates){
    results_obj_rep <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_primary_moderator_prior_self_efficacy_and_any_response_2qs", "_replicate_", idx_replicate, ".rds", sep = "")))
...
write.csv(fit_pooled_causal_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_primary_causal_moderator_prior_self_efficacy_and_any_response_2qs.csv"), row.names = TRUE)
write.csv(fit_pooled_control_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_primary_control_moderator_prior_self_efficacy_and_any_response_2qs.csv"), row.names = TRUE)
write.csv(dat_pbcom_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pbcom_primary_moderator_prior_self_efficacy_and_any_response_2qs.csv"), row.names = TRUE)
write.csv(dat_pool_stats_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pool_stats_primary_causal_moderator_prior_self_efficacy_and_any_response_2qs.csv"), row.names = TRUE)
