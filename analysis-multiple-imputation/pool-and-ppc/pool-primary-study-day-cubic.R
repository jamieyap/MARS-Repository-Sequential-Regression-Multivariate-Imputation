...
results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", 1, "results_obj_primary_study_day_cubic.rds"))
...
for(j in 1:num_terms){
  ...
  for(mi_dataset_num in 1:.__total_imputed_datasets){
    results_obj <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, "results_obj_primary_study_day_cubic.rds"))
...
row.names(fit_pooled) <- c("Treatment (Prompt = 1, No Prompt = 0)", "Treatment x Day", "Treatment x Day Squared", "Treatment x Day Cubed", paste("Treatment Effect on Day ", 1:8, sep = ""))
...
for(mi_dataset_num in 1:.__total_imputed_datasets){
  ...
  for(idx_replicate in 1:.__par_total_replicates){
    results_obj_rep <- readRDS(file = file.path(path_multiple_imputation_pipeline_data, "mi-analysis-results", mi_dataset_num, paste("results_obj_primary_study_day_cubic", "_replicate_", idx_replicate, ".rds", sep = "")))
...
write.csv(fit_pooled_causal_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_primary_causal_study_day_cubic.csv"), row.names = TRUE)
write.csv(fit_pooled_control_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pooled_primary_control_study_day_cubic.csv"), row.names = TRUE)
write.csv(dat_pbcom_formatted, file = file.path("analysis-multiple-imputation", "formatted-output", "pbcom_primary_study_day_cubic.csv"), row.names = TRUE)

###############################################################################
# Workflow: Plot
###############################################################################

png(file = file.path("analysis-multiple-imputation", "formatted-output", "primary_study_day_cubic.png"), width = 6, height = 6, units = "in", res = 600)

plot(1:8, fit_pooled_causal[-c(1:4),]$Estimate, type = "o", lwd = 1, ylim = c(-1,1), xlab = "Day", ylab = "Treatment Effect")
lines(1:8, fit_pooled_causal[-c(1:4),]$LCL90, type = "o", lty = 2, lwd = 1)
lines(1:8, fit_pooled_causal[-c(1:4),]$UCL90, type = "o", lty = 2, lwd = 1)
